...
class DedalApp(ctk.CTk):
    def __init__(self):
        ...
        self.after(201, lambda :self.iconbitmap('data/wings_4.ico'))
...
class NozzleWindow(ctk.CTk):
    ...
    def place_label(self):
        ...
        self.label4 = create_label(self.frame2, "Значение Fотн =: ", 10, 60)
        ...
        self.label22 = create_label(self.frame8, "", 5, 15)
        self.label23 = create_label(self.frame8, "", 5, 60)
        self.label24 = create_label(self.frame8, "", 5, 105)
        ...
        self.label34 = create_label(self.frame7, "50", 50, 105)
        self.label28 = create_label(self.frame7, "80", 235, 105)
        self.label28 = create_label(self.frame9, "", 10, 185)
        self.label29 = create_label(self.frame9, "", 10, 215)
    ...
    def place_button(self):
        ...
        self.true_nozzle = create_button(self.frame9, "Расчёт", lambda: self.print_true_nozzle(), self.font1, 100, 100, 160)
    def back_window(self):
        self.destroy()
        user.F_kp = None
        user.F_ks = None
    ...
    def place_slider(self):
        ...
        self.slider_3 = ctk.CTkSlider(self.frame7, from_=50, to=80, command=self.on_slider_change_3,number_of_steps=30, border_width=4, width=150, height=15, fg_color=("#474747"),progress_color=("#0094FF"))
    ...
    def on_slider_change_3(self, value):
        # Обновление текста метки в соответствии со значением ползунка
        self.label24.configure(text=f"α = {value}°")
        self.kost_3=value
...
class CombustionChamberWindow(ctk.CTk):
    ...
    def print_engine(self):
        ...
        self.image_label.configure(text="")
    def place_slider(self):
        self.slider = ctk.CTkSlider(self.frame0, from_=1.5, to=5,command=self.on_slider_change,number_of_steps=70,border_width=4, width=300, height=15,fg_color=("#474747"),progress_color=("#0094FF"))
        self.slider.place(x=370,y=348)
    def on_slider_change(self, value):
        # Обновление текста метки в соответствии со значением ползунка
        self.label1.configure(text=f"Условное время пребывания: {value:.2f} мс")
        self.tau_pr=value
        self.L_ks=1000*(((self.tau_pr*0.001*self.R_k*self.T_k*self.F_kp)/self.B)-self.V_suzh)/self.F_ks
        self.label4.configure(text=f"Длина камеры сгорания: {self.L_ks:.2f} мс")
    ...
    def place_label(self):
        ...
        self.label2 = create_label_0(self.frame0, f"1.50", 330, 340)
        self.label3 = create_label_0(self.frame0, f"5.00", 680, 340)
    ...
    def back_window(self):
        ...
        user.L_ks = None
        user.teta_a = None
        print(user.R_1, user.R_2, user.Rad_ks, user.Rad_kp, user.alpha_rad_kon, user.aar)
...
class LavalWindow(ctk.CTk):
    def __init__(self, teta_a,teta_m,Rad_kp,Rad_a,x_dozv,y_dozv,L_ks):
        ...
        self.a,self.b,self.c=find_coef_par(self.Rad_kp, self.Rad_a, self.teta_a, self.teta_m)
        self.font1 = ("Futura PT Book", 16)  # Настройка пользовательского шрифта 1
        self.font2 = ("Futura PT Book", 14)  # Настройка пользовательского шрифта 2
        ...
        self.place_button()
        self.place_label()
        self.x_total, self.y_total,self.x_sv,self.y_sv=plot_nozzle_laval(self.Rad_kp, self.Rad_a, self.teta_m, self.teta_a, self.frame0,self.a,self.b,self.c,self.x_dozv,self.y_dozv,self.L_ks)
        user.x_sv=self.x_sv
        user.y_sv =self.y_sv
...
class GraphWindow(ctk.CTk):
    ...
    def calculation_dozv(self):
        ...
        self.f=interp1d(user.y_suzh, user.x_suzh, kind='nearest')
        self.f_1 = interp1d(user.y_sv, user.x_sv, kind='nearest')
        ...
        for r,w in zip(self.rho_array, self.w_array):
            self.rw.append(r*w)
        print_graph_p_x(self.x_graph,self.p_graph,user.L_ks,self.frame0,30,50,"Изменение давления по длине сопла",'х, мм',"p, МПа")
        print_graph_p_x(self.x_graph,self.T_array,user.L_ks,self.frame0,30,750,"Изменение температуры по длине сопла",'х, мм',"Т, К")
        print_graph_p_x(self.x_graph,self.rho_array,user.L_ks,self.frame0,30,1450,"Изменение плотности по длине сопла",'х, мм',"rho, кг/м3")
        print_graph_p_x(self.x_graph,self.w_array,user.L_ks,self.frame0,30,2150,"Изменение скорости по длине сопла",'х, мм',"w, м/с")
        print_graph_p_x(self.x_graph, self.M_array, user.L_ks, self.frame0, 30, 2850,"Изменение числа Маха по длине сопла", 'х, мм', "М")
...
class LossesWindow(ctk.CTk):
    def __init__(self,x_dzv,r_dzv,T_dozv,rho_dozv,R_dozv,k_dozv,w_dozv,F_dozv,x_svzv,M_dozv,r_sv,T_sv_array,rho_sv_array,R_sv_array,k_sv_array,w_sv_array,M_sv,F_sv_array):
        ...
        self.resizable(False, False)  # Запрет изменения размера окна
        self.after(201, lambda: self.iconbitmap('data/wings_1.ico'))
        screen_width = self.winfo_screenwidth()  # Устанавливаем размер и позицию окна
        screen_height = self.winfo_screenheight()
        window_width = 1305
        window_height = 734
    ...
    def poteri_dozvuk(self):
        ...
        self.T_j=srednee_znachenie(self.T_dozv)
        self.M_j = srednee_znachenie(self.M_dozv)
        self.k_j = srednee_znachenie(self.k_dozv)
        self.F_j = srednee_znachenie_F(self.F_dozv, user.m_sum)
        self.C_fi_dozv=[0]*len(self.rho_j)
        self.tau_tr_dozv = [0]*len(self.rho_j)
        self.delta_P_suzh_j=[0]*len(self.rho_j)
        self.Delta_P_suzh=0
        for i in range(0,len(self.rho_j)):
            self.C_fi_dozv[i]=(0.005*((1000/self.T_j[i])**(-0.35))*((1+0.44*(self.k_j[i]-1)*(self.M_j[i]**2))**(-0.55)))
            self.tau_tr_dozv[i]=self.C_fi_dozv[i]*self.rho_j[i]*(self.w_j[i]**2)*0.5
    ...
    def poteri_sverhzvuk_profil(self):
        ...
        self.T_j_sv = srednee_znachenie(self.T_sv_array)
        self.M_j_sv = srednee_znachenie(self.M_sv)
        self.k_j_sv = srednee_znachenie(self.k_sv_array)
        self.F_j_sv = srednee_znachenie_F(self.F_sv_array, user.m_sum)
        self.C_fi_sv = [0] * len(self.rho_sv_array)
        self.tau_tr_sv = [0] * len(self.rho_sv_array)
        self.delta_P_sv = [0] * len(self.rho_sv_array)
        self.Delta_P_rassh = 0
        for i in range(0,len(self.rho_j_sv)):
            self.C_fi_sv[i]=(0.005*((1000/self.T_j_sv[i])**(-0.35))*((1+0.44*(self.k_j_sv[i]-1)*(self.M_j_sv[i]**2))**(-0.55)))
            self.tau_tr_sv[i]=self.C_fi_sv[i]*self.rho_j_sv[i]*(self.w_j_sv[i]**2)*0.5
            self.delta_P_sv[i]=self.F_j_sv[i]*self.tau_tr_sv[i]*(1/math.tan(self.beta_j_sv[i]))
            self.Delta_P_rassh+=self.delta_P_sv[i]
    ...
    def poteri_konich(self):
        ...
        self.Delta_P_kon=[]
        self.C_fi_kon=[0]*len(self.rho_j_sv)
        self.tau_tr_kon=[0]*len(self.rho_j_sv)
        self.Delta_P_rassh_kon=0
        self.delta_P_kon=[0]*len(self.rho_j_sv)
        for beta in self.beta_kon:
            for i in range(0, len(self.rho_j_sv)):
                self.C_fi_kon[i]=(0.005*((1000/self.T_j_sv[i])**(-0.35))*((1+0.44*(self.k_j_sv[i]-1)*(self.M_j_sv[i]**2))**(-0.55)))
                self.tau_tr_kon[i]=self.C_fi_kon[i]*self.rho_j_sv[i]*(self.w_j_sv[i]**2)*0.5
                self.delta_P_kon[i]=self.F_j_sv[i]*self.tau_tr_kon[i]*(1/math.tan(beta))
...
================Коническое сопло================
phi_s={self.max_phi}
угол полураскрытия = {self.beta_kon_itog}
''')
        print_graph_phi_x(self.phi_r,self.phi_tr,self.phi_s, self.beta_kon_grad, self.frame0, 50, 340, "Коэффициенты потерь",'', "phi")
        self.x_total_kon,self.y_total_kon=print_graph_kon(user.x_suzh,user.y_suzh,user.L_ks,self.beta_kon_itog,user.Rad_a,user.Rad_kp,self.frame0, 5, 1080,"Сопло с конической расширяющейся частью", 'х,мм', "r,мм")
    def place_label(self):
        self.text=f'''================Профилированное сопло================
Потери в сужающейся части равны = {self.Delta_P_suzh:.1f} Н
Потери в расширяющейся части для профиллированного сопла равны ={self.Delta_P_rassh:.1f} Н
